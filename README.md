...
Version **0.2.2**；需要 Core `>=0.2.15 <0.3.0` 和公开 Go SDK v0.2.15。
...
版本 0.2.2 没有业务配置字段。`app_config` 必须是空 JSON object：
...
- 0.2.2 不猜测 `display-text@1` 或 `led@1` 的 action/args 协议，因此即使绑定存在，
...
版本 0.2.2 已接受并保留两个可选绑定，但没有假定其硬件语义。后续版本可以在不改变
`tone` 契约的前提下增加：
